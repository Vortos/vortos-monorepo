...
final class Version20260430000001 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql(<<<'SQL'
            CREATE INDEX IF NOT EXISTS idx_vortos_outbox_status_next
                ON vortos_outbox (status, next_attempt_at)
                WHERE status = 'pending'
        SQL);

        $this->addSql(<<<'SQL'
            CREATE INDEX IF NOT EXISTS idx_vortos_outbox_aggregate_occurred
                ON vortos_outbox (aggregate_id, occurred_at)
        SQL);

        $this->addSql(<<<'SQL'
            CREATE INDEX IF NOT EXISTS idx_vortos_outbox_type_occurred
