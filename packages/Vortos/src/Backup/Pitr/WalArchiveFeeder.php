    ...
    private function serve(ContainerHandle $handle, string $segment): bool
    {
        ...
        for ($attempt = 1; $attempt <= $attempts; $attempt++) {
            try {
                ...
                // Checked before the first retry, not after the last: a history file is absent on
                // every cluster that has never diverged, so burning the backoff ladder would add
                // seconds to the end of every recovery for an outcome that was never in doubt.
                if (!$isSegment) {
                    $this->markAbsent($handle, $segment);
