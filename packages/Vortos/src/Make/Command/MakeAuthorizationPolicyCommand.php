...
#[AsCommand(
    name: 'vortos:make:authorization-policy',
    description: 'Generate a resource authorization policy',
)]
final class MakeAuthorizationPolicyCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $output->writeln("<info>vortos:make:authorization-policy</info> {$name} --context={$context}");
        $output->writeln('');

        $this->engine->write(
            "{$context}/Application/Policy/{$name}Policy.php",
            $this->engine->render('authorization-policy', $vars),
            $output,
        );
