...
#[AsCommand(
    name: 'vortos:make:rate-limit-policy',
    description: 'Generate a rate limit policy',
)]
final class MakeRateLimitPolicyCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $output->writeln("<info>vortos:make:rate-limit-policy</info> {$name} --context={$context}");
        $output->writeln('');

        $this->engine->write(
            "{$context}/Application/Policy/{$name}RateLimitPolicy.php",
            $this->engine->render('rate-limit-policy', $vars),
            $output,
        );
