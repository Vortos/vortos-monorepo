...
return [
    ...
    'observability' => [
        ...
        'commands' => [
            ...
            'vortos:observability:collector' => 'Generate collector config from the selected MetricsSinkInterface driver. Flags: --sink, --bind (receiver host), --storage-dir, --force, --dry-run.',
    ...
    'release' => [
        ...
        'provides'    => [
            ...
            'AvailableMigrationSetReaderInterface' => 'Reads the migrations AVAILABLE in the build (on disk), not the applied set — the schema fingerprint recorded for a build must be the desired schema it carries, because record-manifest runs before those migrations apply. DoctrineAvailableMigrationSetReader is the default.',
        ...
        'commands' => [
            ...
            'vortos:release:record-manifest' => 'Record the immutable BuildManifest for an environment AFTER build+push and BEFORE deploy — this is what makes a desired build resolvable by `deploy`. Derives the schema fingerprint from the build\'s available migrations. Idempotent on build id. Flags: --env, --repository, --digest, --git-sha, --build-id, --arch, --builder-id, --base-image-digest, --json. Emitted automatically by the generated deploy job.',
        ],
    ],
    ...
    'pipeline' => [
        ...
        'provides'    => [
            ...
            'Generated deploy job'              => 'Deploy-on-target: the runner opens SSH to the VPS and runs vortos:release:record-manifest → vortos:deploy:provision → deploy:doctor → deploy INSIDE the pulled image ON the VPS, attached to the app Docker network (app_network). This fixes two things at once: the control-plane commands reach the prod release-ledger DB by construction, and the arm64 image runs on the arm64 host — the runner runs only an ssh client and never `docker run`s the image (no runner/image arch mismatch). runs-on: ubuntu-latest; requests id-token: write iff oidc. Posture-aware auth: OIDC path = zero standing secrets beyond GITHUB_TOKEN (SSH cert federated from the id-token via non-secret vars.*, enforced by OidcZeroStandingSecretTest); ssh-key path (oidc: false) authenticates with secrets.VORTOS_DEPLOY_SSH_KEY and forwards the age KEK VORTOS_AGE_IDENTITY (the age-encrypted store is delivered to the deploy dir and mounted ro). Deploy runs local-mode on the VPS (no nested SSH).',
    ...
    'deploy' => [
        ...
        'provides'    => [
            ...
            'ArtifactDelivery (G3)'        => 'Ships the runtime files that live OUTSIDE the image — .env.prod, docker-compose.prod.yaml, docker/** + observability/** config trees, and the age-encrypted vortos-secrets.age — to the remote deploy dir over the SSH transport. DeliveryManifest::default() declares the set; required artifacts fail closed before anything is sent; secret files ship 0600; the tree is staged then swapped atomically (no partial-config window); remote paths are traversal-guarded. Registered whenever push-mode SSH is wired.',
            'FirstDeployProvisioner (G4)'  => 'Pure planner behind vortos:deploy:provision — keys-if-absent → migrate --force → secrets preflight, ordered and idempotent. The command runs the plan via the Application.',
        ],
        'config'   => [
        ...
        'commands' => [
            ...
            'vortos:deploy:provision' => 'Idempotent first-deploy provisioning (G4), invoked inside the image on the VPS by the generated deploy job before deploy:doctor: generate RS256 JWT keys ONLY when absent (checked via JWT_PRIVATE_KEY_PATH/JWT_PUBLIC_KEY_PATH) → vortos:migrate --force → secrets:preflight. Safe to run on every deploy. Flags: --env, --json.',
