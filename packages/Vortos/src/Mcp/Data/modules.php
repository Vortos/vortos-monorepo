...
return [
    ...
    'deploy' => [
        ...
        'provides'    => [
            ...
            'BackupToolchainCheck (F6-1)'  => 'Deploy-time bridge to vortos-backup\'s BackupToolchainInspector, registered into deploy:doctor guarded by class_exists (same pattern as MigrationDriftCheck). Skips when VORTOS_BACKUP_ENGINE is unset (deploy does not force backups); FAILS deploy:doctor closed when an engine IS configured but its client binaries (pg_dump/pg_restore/pg_basebackup) are missing or older than the server major in the image that runs backups — so a broken backup toolchain is caught before cutover, not at the first real backup. Same probe as the backup:doctor command.',
