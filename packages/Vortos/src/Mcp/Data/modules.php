...
return [
    ...
    'secrets' => [
        ...
        'config'   => [
            ...
            'VORTOS_SECRETS_STORE_PATH / _AGE_PUBLIC_KEY / _AGE_IDENTITY' => 'File store path + age key custody (public key baked in build; private identity read from env at use-time).',
        ],
        'commands' => [
    ...
    'pipeline' => [
        ...
        'provides'    => [
            ...
            'Generated deploy job'              => 'Deploy-in-image: runs vortos:release:record-manifest → deploy:doctor → deploy via `docker run <repo>@<digest> php bin/console ...`, so the app\'s config/keys are present by construction (no PHP/composer/keys on the runner). Permissions request id-token: write iff oidc. Posture-aware secrets: the OIDC path references zero standing secrets (only GITHUB_TOKEN, enforced by OidcZeroStandingSecretTest); the ssh-key path (oidc: false) exposes the single age KEK secret VORTOS_AGE_IDENTITY and mounts the committed age-encrypted store.',
        ...
        'config'   => [
            'config/pipeline.php' => 'Returns an associative array (or Closure returning one) of overrides: workflow_filename, workflow_name, test_command, analyse_command, test_steps (list of [name,run]), test_service_containers (list of ServiceContainer or [name,image,ports,env,options]), image_repository, registry_provider, environments, php_extensions, etc.',
            'env'                 => 'Scalars also settable via env: PIPELINE_WORKFLOW_FILENAME, PIPELINE_WORKFLOW_NAME, PIPELINE_TEST_COMMAND, PIPELINE_ANALYSE_COMMAND, PIPELINE_IMAGE_REPOSITORY, PIPELINE_REGISTRY_PROVIDER, PIPELINE_PHP_VERSION, PIPELINE_ENVIRONMENTS, PIPELINE_EMIT_SBOM/SCAN_GATE/SIGN. config/pipeline.php wins over env.',
        ],
        'commands' => [
