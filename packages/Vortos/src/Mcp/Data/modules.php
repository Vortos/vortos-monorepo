...
return [
    ...
    'scheduler' => [
        ...
        'description' => 'Distributed, multi-tenant, enterprise-grade job scheduler. Cron and interval triggers, distributed lease (SQL/Redis/Postgres Advisory) for single-execution guarantees, misfire recovery, overlap policies, RBAC+4-eyes security, HMAC hash-chain audit trail, 8 Prometheus metrics, OTel spans, circuit breaker, dead-man detector, and an optional HTMX+Twig admin UI. All S0–S11 blocks complete as of 2026-07.',
        'provides'    => [
            'SchedulerDaemon'           => 'Long-lived process started via scheduler:run. Tick loop: acquire lease → DueScan → MisfireResolver → SlotCalculator → FireDispatcher (atomic claim+enqueue) → sleep. Type FireDispatcherPort — accepts DispatchCircuitBreaker wrapper.',
            'ScheduleService / ScheduleServiceInterface' => 'Runtime CRUD for dynamic schedules. create/pause/resume/delete/fireNow/list/get — all writes check CommandSpecValidator (allowlist) and SchedulePolicyInterface (RBAC+4-eyes) before persisting.',
            ...
            '#[SchedulableCommand]'     => 'PHP attribute on a command class. CommandSpecValidator rejects any attempt to schedule a class without this tag (CommandNotAllowlistedException). The allowlist prevents arbitrary code execution via the scheduler API.',
            ...
            'LeasePort'                 => 'Distributed mutex port. Drivers: SqlLeaseStore (default — INSERT ON CONFLICT DO UPDATE WHERE expires_at < NOW()), RedisLeaseStore (SET NX PX), PostgresAdvisoryLeaseStore (pg_try_advisory_lock — zero-TTL, releases on disconnect). Selected via SCHEDULER_LEASE_DRIVER env var + LeaseDriverPass.',
            ...
            'DispatchCircuitBreaker'    => 'Wraps FireDispatcherPort. Opens after SCHEDULER_CB_FAILURE_THRESHOLD consecutive failures (default 5). Returns CircuitOpen without calling inner. Probes in HalfOpen after SCHEDULER_CB_RECOVERY_WINDOW_SEC (default 30s). Returns to Closed on successful probe.',
            ...
            'SchedulerAuditProjector'   => 'Append-only HMAC hash-chain audit log. Every schedule mutation and every fire produces an SchedulerAuditEntry. Chain keyed per-tenant. SchedulerAuditCheckpointProjector writes periodic checkpoints so verification is O(n/epochSize) not O(n).',
            'SchedulerAuditChainVerifier' => 'Verifies chain integrity from a known-good checkpoint. Returns SchedulerChainVerificationResult with isValid(), brokenAtEntryId, entryCount. Run via scheduler:doctor check C8.',
            'CardinalityGuardedSchedulerMetrics' => 'Decorator over SchedulerMetricsPort. Caps distinct schedule_id label values at SCHEDULER_METRICS_MAX_CARDINALITY (default 200). Overflow fires use __overflow__ label and increment vortos_scheduler_metric_overflow_total. Prevents Prometheus OOM in multi-tenant deployments.',
            'DeadManDetector'           => 'Exposes vortos_scheduler_dead_man_last_tick_age_seconds gauge. Rises if daemon stops ticking. Doctor check C9 fails if age > SCHEDULER_DEAD_MAN_WINDOW_SEC.',
            'SchedulerTracer'           => 'OTel span per fire: scheduler.fire, attributes: schedule_id, schedule_name, tenant_id, slot, result, fire_duration_ms.',
            'SchedulerDoctor / SchedulerDoctorPort' => '9-check health suite: C1 schedule store, C2 run store, C3 lease driver, C4 audit table, C5 static schedule registered, C6 no permanent error state, C7 approval store (skip if absent), C8 audit chain integrity, C9 dead-man last tick. Exit code 0 = all pass, 1 = any fail.',
            ...
            'SchedulePolicy / SchedulePolicyInterface' => 'RBAC gate for all schedule operations. Default implementation integrates with Vortos auth engine. Permissions: scheduler.schedule.create/pause/resume/delete/fire_now/read, scheduler.approval.approve/reject. NullSchedulePolicy allows all (internal services only).',
            ...
            'SlotCalculator'            => 'Produces content-addressed slot key: {scheduleId}:{ISO8601_fire_time}. Slot keys are the fundamental idempotency unit — same schedule + same fire time always produce the same key.',
        ...
        'config' => [
            'SCHEDULER_LEASE_DRIVER'                => 'sql (default), redis, postgres_advisory',
            'SCHEDULER_SHARD_COUNT'                 => 'int, default 1. Shard count — one daemon per shard, stable schedule→shard mapping via crc32(id) % count.',
            'SCHEDULER_IDLE_SLEEP_SEC'              => 'int, default 10. Seconds between daemon ticks.',
            ...
            'SCHEDULER_DEAD_MAN_WINDOW_SEC'         => 'int, default 120. Alert if no tick within this window.',
            'SCHEDULER_METRICS_MAX_CARDINALITY'     => 'int, default 200. Max distinct schedule_id Prometheus label values.',
        ],
        'commands' => [
            'scheduler:run'     => 'Start the daemon. Long-lived process. Options: --shard=N. Exits cleanly on SIGTERM after completing the current tick.',
            ...
            'scheduler:run-now' => 'Manually trigger an immediate fire outside the normal schedule. Recorded as ManualFire in audit log.',
            ...
            'scheduler:prune'   => 'Prune old run history. Options: --older-than (default: 90days)',
            'scheduler:doctor'  => 'Run 9-check health suite. Exit 0 = all pass, 1 = any fail. Check IDs: C1–C9.',
        ],
    ],
