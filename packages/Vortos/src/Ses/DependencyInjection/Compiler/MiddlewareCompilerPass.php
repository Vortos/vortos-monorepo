...
use Vortos\Ses\Contract\EmailSendObserverInterface;
 ...
 *   Collects all services tagged 'vortos_ses.email_middleware'.
 ...
 *
 * Observer discovery:
 *   Collects all services tagged 'vortos_ses.send_observer'.
...
final class MiddlewareCompilerPass implements CompilerPassInterface
{
    ...
    {
        $tagged = $container->findTaggedServiceIds('vortos_ses.email_middleware');
    ...
    private function wireObservers(ContainerBuilder $container): void
    {
        ...
        $tagged = $container->findTaggedServiceIds('vortos_ses.send_observer');
