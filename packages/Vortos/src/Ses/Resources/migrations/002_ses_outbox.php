...
return new class extends AbstractModuleSchemaProvider {
    ...
    {
        $table = $schema->createTable('ses_outbox');
        ...
        $table->addUniqueIndex(['domain_event_id'], 'uq_ses_outbox_domain_event_id');

        // Relay worker query: pending rows ready to process, skip locked for parallel workers
        $table->addIndex(['status', 'next_attempt_at'], 'idx_ses_outbox_relay', [], [
            'where' => "status = 'pending'",
        ]);

        // Dashboard / monitoring: all rows for a given status
        $table->addIndex(['status', 'created_at'], 'idx_ses_outbox_status_created');
    }
};
