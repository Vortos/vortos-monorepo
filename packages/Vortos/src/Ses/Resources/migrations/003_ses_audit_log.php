...
return new class extends AbstractModuleSchemaProvider {
    ...
    {
        $table = $schema->createTable('ses_audit_log');
        ...
        $table->addUniqueIndex(['message_id'], 'uq_ses_audit_log_message_id');

        // Lookup by outbox row (for relay correlation)
        $table->addIndex(['outbox_id'],         'idx_ses_audit_log_outbox');
        $table->addIndex(['sent_at'],            'idx_ses_audit_log_sent_at');
    }
};
